...
resource "google_sql_database_instance" "main" {
  name             = "zenwebapi10-db-${random_id.db_name_suffix.hex}"
...
output "connection_string" {
  ...
  value = var.database_type == "mysql" ? "Server=${google_sql_database_instance.main.public_ip_address};Database=${google_sql_database.database.name};Uid=${google_sql_user.user.name};Pwd=${random_password.database_password.result};" : "Host=${google_sql_database_instance.main.public_ip_address};Database=${google_sql_database.database.name};Username=${google_sql_user.user.name};Password=${random_password.database_password.result};"
  sensitive = true
}
